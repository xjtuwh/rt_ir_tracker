...
class GDHW(BaseDataset):

    def __init__(self, root=None, image_loader=default_image_loader, set_ids=None):

        root = env_settings().gdhw_dir if root is None else root
    ...
    def _get_sequence_path(self, seq_id):
        ...
        seq_path = os.path.join(self.root, seq_name, 'img')
        anno_path = os.path.join(self.root, seq_name, 'groundtruth_rect.txt')
    ...
    def _get_frame_path(self,seq_path,frame_id):
        ...
        return os.path.join(seq_path, '{:05}.jpg'.format(frame_id + min(result)))
