...
from ltr.dataset import Lasot, Got10k, TrackingNet, MSCOCOSeq, visdrone, VisdroneVal, VOTIR_TRAIN, VOTIR_VAL, IR234, UAV, GDHW
...
def run(settings):
    ...
    proposal_params = {'min_iou': 0.2, 'boxes_per_frame': 8, 'sigma_factor': [0.01, 0.05, 0.1, 0.2, 0.3]}
    # proposal_params = {'min_iou': 0, 'boxes_per_frame': 8, 'sigma_factor': [0.01, 0.05, 0.1, 0.2, 0.3]}
    ...
    dataset_train = sampler.DiMPSampler([lasot_train, got10k_train, trackingnet_train, coco_train, ir_train, uav_train, vot_ir_train, gdhw], [1, 1, 1, 1, 1, 1, 1, 1],
                                        samples_per_epoch=10000, max_gap=30, num_test_frames=3, num_train_frames=3,
                                        processing=data_processing_train)
    # dataset_train = sampler.DiMPSampler([gdhw], [1],
    #                                     samples_per_epoch=13000, max_gap=30, num_test_frames=3, num_train_frames=3,
    #                                     processing=data_processing_train)
    ...
    dataset_val = sampler.DiMPSampler([ir_val], [1], samples_per_epoch=5000, max_gap=30,
    ...
    loss_weight = {'iou': 10, 'test_clf': 100, 'test_init_clf': 100, 'test_iter_clf': 400}

    actor = actors.DiMPActor(net=net, objective=objective, loss_weight=loss_weight)

    # Optimizer
    optimizer = optim.Adam([{'params': actor.net.classifier.filter_initializer.parameters(), 'lr': 2.5e-5},
                            {'params': actor.net.classifier.filter_optimizer.parameters(), 'lr': 2.5e-4},
                            {'params': actor.net.classifier.feature_extractor.parameters(), 'lr': 2.5e-5},
                            {'params': actor.net.bb_regressor.parameters()},
                            {'params': actor.net.feature_extractor.parameters(), 'lr': 1e-5}],
                           lr=1e-4)
