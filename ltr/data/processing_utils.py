...
def iou(reference, proposals):
    ...
    # tl_c = torch.min(reference[:,:2], proposals[:,:2])
    # br_c = torch.max(reference[:,:2] + reference[:,2:], proposals[:,:2] + proposals[:,2:])
    # sz_c = br_c - tl_c
    # region_c = sz_c.prod(dim=1)

    # giou = iou - (region_c - union) * 1.0 / region_c
    return iou
