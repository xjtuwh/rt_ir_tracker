...
def main(mode='IR', visulization=False):
    ...
    # setup tracker
    output_dir = os.path.join('results', 'dimp_det')
                ...
                if tracker.debug_info['max_score'] < th_low * max_score_ref:
                    ...
                    if max_response > th_pass * max_score_ref:
                        ...
                        if out_det[0][:, -1].size > 0:
                            ...
                            if out_det[0][idx][-1] >= 0.1:
                                x, y, w, h = int(out_det[0][idx][0]),  int(out_det[0][idx][1]), int(out_det[0][idx][2] -  out_det[0][idx][0]), int(out_det[0][idx][3] - out_det[0][idx][1])
                                out = [x, y, w, h]
