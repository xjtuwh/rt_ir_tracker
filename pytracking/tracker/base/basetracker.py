...
class BaseTracker:
    ...
    def track_sequence(self, sequence):
        ...
        for frame in sequence.frames[1:]:
            ...
            # else:
            #     self.save(image, out['target_bbox'], frame, sequence.name)
    ...
    def save(self, image, state, frame, name):
        self.fig, self.ax = plt.subplots(1)
        self.ax.cla()
        self.ax.imshow(image)
        rect = patches.Rectangle((state[0], state[1]), state[2], state[3], linewidth=1, edgecolor='r', facecolor='none')
        ...
        self.ax.set_axis_off()
        self.ax.axis('equal')
        if not (os.path.exists('tracking_results/wh_save/' + name)):
            os.makedirs('tracking_results/wh_save/' + name)
        self.fig.savefig('tracking_results/wh_save/' + name + '/' + frame[-12:])
