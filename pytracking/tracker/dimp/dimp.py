...
class DiMP(BaseTracker):
    ...
    def track(self, image) -> dict:
        ...
        if self.target_sz[0] < im.shape[2] and self.target_sz[1] < im.shape[3]:
            new_state = torch.cat((self.pos[[1,0]] - (self.target_sz[[1,0]]-1)/2, self.target_sz[[1,0]]))
        else:
            new_state = torch.cat((self.pos[[1, 0]] - (self.target_sz[[1, 0]] - 1) / 2, torch.Tensor([im.shape[3], im.shape[2]])))
    ...
    def refine_target_box(self, backbone_feat, sample_pos, sample_scale, scale_ind, update_scale = True):
        ...
        if self.params.num_init_random_boxes > 0:
            ...
            new_sz = (init_box[2:] + rand_bb[:,2:]).clamp(minimal_edge_size, init_box[2:].max()*3)
