...
class ATOM(BaseTracker):
    ...
    def track(self, image) -> dict:
        ...
        PSR = psr(scores_raw[0])

        # Update position and scale  and PSR > 100
        if flag != 'not_found' and PSR > 100:
            if self.use_iou_net:
        ...
        # out = {'target_bbox': new_state.tolist(), 'PSR': PSR}
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        if 'flipud' in self.params.augmentation and self.params.augmentation['flipud']:
            self.transforms.append(augmentation.FlipVertical(aug_output_sz, get_rand_shift()))
        ...
        if 'grid' in self.params.augmentation and self.params.augmentation['grid']:
            self.transforms.append(augmentation.GridMask(aug_output_sz, get_rand_shift()))
