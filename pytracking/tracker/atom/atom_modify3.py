...
class ATOM_MODIFY3(BaseTracker):
    ...
    def track(self, image, image_rgb, tracker_rgb) -> dict:
        self.debug_info = {}
        ...
        self.im_ir = im  # For debugging only
        im_rgb = numpy_to_torch(image_rgb)
        self.im_rgb = im_rgb  # For debugging only
        ...
        sample_scales_rgb = tracker_rgb.target_scale * self.params.scale_factors

        test_x = self.extract_processed_sample(im, im_rgb, self.pos, tracker_rgb.pos, sample_scales, sample_scales_rgb, self.img_sample_sz)
    ...
    def extract_sample(self, im_ir: torch.Tensor, im_rgb: torch.Tensor, pos_ir: torch.Tensor, pos_rgb: torch.Tensor,
                       scales_ir, scales_rgb, sz: torch.Tensor):
        feature_map_ir = self.params.features.extract(im_ir, pos_ir, scales_ir, sz)[0]
        feature_map_rgb = self.params.features.extract(im_rgb, pos_rgb, scales_rgb, sz)[0]
        return TensorList([torch.cat((feature_map_ir[0], feature_map_rgb[0]), dim=1)]).unroll()
    ...
    def generate_init_samples(self, im: torch.Tensor) -> TensorList:
        ...
        init_samples = self.params.features.extract_transformed(im, self.im_rgb, self.pos, self.pos_rgb,
                                                                self.target_scale, self.target_scale_rgb,
