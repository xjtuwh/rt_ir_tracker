...
class ATOM_MODIFY2(BaseTracker):
    ...
    def track(self, image_ir, image_rgb, tracker_rgb) -> dict:
        self.debug_info = {}
        ...
        sample_pos_rgb = tracker_rgb.pos_ir.round()
        sample_scales_rgb = tracker_rgb.target_scale_ir * self.params.scale_factors
        test_x = self.extract_processed_sample(im_ir, im_rgb, self.pos_ir, tracker_rgb.pos_ir, sample_scales_ir, sample_scales_rgb, self.img_sample_sz_ir)
        ...
        if flag != 'not_found':
            if self.use_iou_net:
                ...
                if getattr(self.params, 'use_classifier', True):
                    self.update_state(sample_pos_ir + translation_vec)
                self.refine_target_box(sample_pos_ir, sample_scales_ir[scale_ind], scale_ind, update_scale_flag)
            elif getattr(self.params, 'use_classifier', True):
                self.update_state(sample_pos_ir + translation_vec, sample_scales_ir[scale_ind])
        ...
        if update_flag:
            ...
            train_y = self.get_label_function(sample_pos_ir, sample_scales_ir[scale_ind])
        ...
        if self.use_iou_net and flag != 'not_found':
            self.pos_ir = self.pos_iounet.clone()

        # Return new state
        new_state = torch.cat((self.pos_ir[[1,0]] - (self.target_sz_ir[[1,0]]-1)/2, self.target_sz_ir[[1,0]]))
    ...
    def inference(self, image) -> dict:
        ...
        score_map = s[scale_ind, ...]
        max_score = torch.max(score_map).item()
        out = {'score': score_map, 'peak': max_score, 'vec': translation_vec, 'flag': flag}
        return out
    ...
    def localize_target(self, scores_raw):
        ...
        translation_vec = disp[scale_ind, ...].view(-1) * (self.img_support_sz / self.output_sz) * self.target_scale_ir
    ...
    def localize_advanced(self, scores):
        ...
        translation_vec1 = target_disp1 * (self.img_support_sz / self.output_sz) * self.target_scale_ir
        ...
        target_neigh_sz = self.params.target_neighborhood_scale * self.target_sz_ir / self.target_scale_ir
        ...
        translation_vec2 = target_disp2 * (self.img_support_sz / self.output_sz) * self.target_scale_ir
    ...
    def extract_processed_sample(self, im_ir: torch.Tensor, im_rgb: torch.Tensor, pos_ir: torch.Tensor,
                                 pos_rgb: torch.Tensor, scales_ir, scales_rgb, sz: torch.Tensor) -> (
    ...
    def generate_init_samples(self, im_ir: torch.Tensor, im_rgb: torch.Tensor) -> TensorList:
        ...
        # im_ir im_rgb
        init_samples = self.params.features.extract_transformed(im_ir, im_rgb, self.pos_ir, self.pos_rgb,
                                                                self.target_scale_ir, self.target_scale_rgb,
    ...
    def init_label_function(self, train_x):
        ...
        self.sigma = (self.feature_sz / self.img_support_sz * self.base_target_sz_ir).prod().sqrt() * output_sigma_factor * torch.ones(2)

        # Center pos in normalized coords
        target_center_norm = (self.pos_ir - self.pos_ir.round()) / (self.target_scale_ir * self.img_support_sz)
    ...
    def update_state(self, new_pos, new_scale = None):
        ...
        if new_scale is not None:
            self.target_scale_ir = new_scale.clamp(self.min_scale_factor_ir, self.max_scale_factor_ir)
            self.target_sz_ir = self.base_target_sz_ir * self.target_scale_ir

        # Update pos
        inside_ratio = 0.2
        inside_offset = (inside_ratio - 0.5) * self.target_sz_ir
        self.pos_ir = torch.max(torch.min(new_pos, self.image_sz_ir - inside_offset), inside_offset)
