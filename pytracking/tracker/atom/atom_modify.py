...
class ATOM_MODIFY(BaseTracker):
    ...
    def initialize(self, image_ir, image_rgb, info_ir: dict, info_rgb) -> dict:
        state_ir = info_ir            # ['init_bbox']
        state_rgb = info_rgb
        ...
        self.params.features.set_is_color(image_rgb.shape[2] == 3)
        ...
        self.pos_ir = torch.Tensor([state_ir[1] + (state_ir[3] - 1)/2, state_ir[0] + (state_ir[2] - 1)/2])
        self.target_sz_ir = torch.Tensor([state_ir[3], state_ir[2]])

        # Get position and size
        self.pos_rgb = torch.Tensor([state_rgb[1] + (state_rgb[3] - 1)/2, state_rgb[0] + (state_rgb[2] - 1)/2])
        self.target_sz_rgb = torch.Tensor([state_rgb[3], state_rgb[2]])

        # Set search area
        self.target_scale_ir = 1.0
        search_area_ir = torch.prod(self.target_sz_ir * self.params.search_area_scale).item()
        if search_area_ir > self.params.max_image_sample_size:
            self.target_scale_ir =  math.sqrt(search_area_ir / self.params.max_image_sample_size)
        elif search_area_ir < self.params.min_image_sample_size:
            self.target_scale_ir =  math.sqrt(search_area_ir / self.params.min_image_sample_size)

        # Set search area
        self.target_scale_rgb = 1.0
        search_area_rgb = torch.prod(self.target_sz_rgb * self.params.search_area_scale).item()
        if search_area_rgb > self.params.max_image_sample_size:
            self.target_scale_rgb =  math.sqrt(search_area_rgb / self.params.max_image_sample_size)
        elif search_area_rgb < self.params.min_image_sample_size:
            self.target_scale_rgb =  math.sqrt(search_area_rgb / self.params.min_image_sample_size)
        ...
        self.base_target_sz_ir = self.target_sz_ir / self.target_scale_ir
        self.base_target_sz_rgb = self.target_sz_rgb / self.target_scale_rgb

        # Use odd square search area and set sizes
        feat_max_stride = max(self.params.features.stride())
        if getattr(self.params, 'search_area_shape', 'square') == 'square':
            self.img_sample_sz_ir = torch.round(torch.sqrt(torch.prod(self.base_target_sz_ir * self.params.search_area_scale))) * torch.ones(2)
        elif self.params.search_area_shape == 'initrect':
            self.img_sample_sz_ir = torch.round(self.base_target_sz_ir * self.params.search_area_scale)
        else:
            raise ValueError('Unknown search area shape')
        if self.params.feature_size_odd:
            self.img_sample_sz_ir += feat_max_stride - self.img_sample_sz_ir % (2 * feat_max_stride)
        else:
            self.img_sample_sz_ir += feat_max_stride - (self.img_sample_sz_ir + feat_max_stride) % (2 * feat_max_stride)

        if getattr(self.params, 'search_area_shape', 'square') == 'square':
            self.img_sample_sz_rgb = torch.round(torch.sqrt(torch.prod(self.base_target_sz_rgb * self.params.search_area_scale))) * torch.ones(2)
        elif self.params.search_area_shape == 'initrect':
            self.img_sample_sz_rgb = torch.round(self.base_target_sz_rgb * self.params.search_area_scale)
        else:
            raise ValueError('Unknown search area shape')
        if self.params.feature_size_odd:
            self.img_sample_sz_rgb += feat_max_stride - self.img_sample_sz_rgb % (2 * feat_max_stride)
        else:
            self.img_sample_sz_rgb += feat_max_stride - (self.img_sample_sz_rgb + feat_max_stride) % (2 * feat_max_stride)

        # Set sizes
        self.img_support_sz = self.img_sample_sz_ir
        self.feature_sz = self.params.features.size(self.img_sample_sz_ir)
        ...
        self.iou_img_sample_sz = self.img_sample_sz_ir
        ...
        # Convert image
        im_rgb = numpy_to_torch(image_rgb)
        self.im_rgb = im_rgb    # For debugging only
        im_ir = numpy_to_torch(image_ir)
        self.im_ir = im_ir    # For debugging only

        # Setup scale bounds
        self.image_sz_ir = torch.Tensor([im_ir.shape[2], im_ir.shape[3]])
        self.min_scale_factor_ir = torch.max(10 / self.base_target_sz_ir)
        self.max_scale_factor_ir = torch.min(self.image_sz_ir / self.base_target_sz_ir)

        self.image_sz_rgb = torch.Tensor([im_rgb.shape[2], im_rgb.shape[3]])
        self.min_scale_factor_rgb = torch.max(10 / self.base_target_sz_rgb)
        self.max_scale_factor_rgb = torch.min(self.image_sz_rgb / self.base_target_sz_rgb)

        # Extract and transform sample
        x = self.generate_init_samples(im_ir, im_rgb)
        ...
        init_y_ir, init_y_rgb = self.init_label_function(train_x)

        # Init memory
        self.init_memory(train_x)

        # Init optimizer and do initial optimization
        self.init_optimization_ir(train_x, init_y_ir)
        self.init_optimization_rgb(train_x, init_y_rgb)

        self.pos_iounet = self.pos_ir.clone()

        out = {'time': time.time() - tic}
        return out

    def init_optimization_ir(self, train_x, init_y_ir):
        # Initialize filter
        ...
        if self.params.update_projection_matrix:
            self.joint_problem = FactorizedConvProblem(self.init_training_samples, init_y_ir, self.filter_reg,
        ...
        # Initialize optimizer
        self.conv_problem = ConvProblem(self.training_samples, self.y_ir, self.filter_reg, self.sample_weights, self.response_activation)
    ...
    def init_optimization_rgb(self, train_x, init_y_rgb):
        # Initialize filter
        filter_init_method = getattr(self.params, 'filter_init_method', 'zeros')
        self.filter = TensorList(
            [x.new_zeros(1, cdim, sz[0], sz[1]) for x, cdim, sz in zip(train_x, self.compressed_dim, self.kernel_size)])
        ...
        # Setup factorized joint optimization
        if self.params.update_projection_matrix:
            self.joint_problem = FactorizedConvProblem(self.init_training_samples, init_y_rgb, self.filter_reg,
                                                       self.fparams.attribute('projection_reg'), self.params, self.init_sample_weights,
                                                       self.projection_activation, self.response_activation)
        ...
        # Initialize optimizer
        self.conv_problem = ConvProblem(self.training_samples, self.y_rgb, self.filter_reg, self.sample_weights, self.response_activation)
    ...
    def track(self, image_ir, image_rgb) -> dict:
        self.debug_info = {}
        ...
        # Get sample
        sample_pos_ir = self.pos_ir.round()
        sample_scales_ir = self.target_scale_ir * self.params.scale_factors
        sample_pos_rgb = self.pos_rgb.round()
        sample_scales_rgb = self.target_scale_rgb * self.params.scale_factors
        test_x = self.extract_processed_sample(im_ir, im_rgb, self.pos_ir, self.pos_rgb, sample_scales_ir, sample_scales_rgb, self.img_sample_sz_ir)

        # Compute scores
        scores_raw = self.apply_filter(test_x)
        translation_vec_ir, translation_vec_rgb, scale_ind, s, flag = self.localize_target(scores_raw)

        # Update position and scale
        if flag != 'not_found':
            self.update_state(self.pos_ir + translation_vec_ir, self.pos_rgb + translation_vec_rgb, sample_scales_ir[scale_ind], sample_scales_rgb[scale_ind])
        ...
        if update_flag:
            ...
            train_y = self.get_label_function(self.pos_ir.round(), sample_scales_ir[scale_ind])
        ...
        # Return new state
        new_state_ir = torch.cat((self.pos_ir[[1,0]] - (self.target_sz_ir[[1,0]]-1)/2, self.target_sz_ir[[1,0]]))
        new_state_rgb = torch.cat((self.pos_rgb[[1, 0]] - (self.target_sz_rgb[[1, 0]] - 1) / 2, self.target_sz_rgb[[1, 0]]))

        out = {'target_bbox': new_state_ir.tolist(),'target_bbox_rgb': new_state_rgb.tolist()}
        return out
    ...
    def localize_target(self, scores_raw):
        ...
        translation_vec_ir = disp[scale_ind, ...].view(-1) * (self.img_support_sz / self.output_sz) * self.target_scale_ir
        translation_vec_ir *= self.params.scale_factors[scale_ind]

        translation_vec_rgb = disp[scale_ind, ...].view(-1) * (self.img_support_sz / self.output_sz) * self.target_scale_rgb
        translation_vec_rgb *= self.params.scale_factors[scale_ind]
        ...
        return translation_vec_ir, translation_vec_rgb, scale_ind, scores, None
    ...
    def localize_advanced(self, scores):
        ...
        translation_vec1_ir = target_disp1 * (self.img_support_sz / self.output_sz) * self.target_scale_ir
        translation_vec1_rgb = target_disp1 * (self.img_support_sz / self.output_sz) * self.target_scale_rgb

        if max_score1.item() < self.params.target_not_found_threshold:
            return translation_vec1_ir, translation_vec1_rgb, scale_ind, scores, 'not_found'

        if self.output_window is not None and getattr(self.params, 'perform_hn_without_windowing', False):
            scores = scores_orig

        # Mask out target rgb neighborhood
        target_neigh_sz_rgb = self.params.target_neighborhood_scale * self.target_sz_rgb / self.target_scale_rgb
        tneigh_top = max(round(max_disp1[0].item() - target_neigh_sz_rgb[0].item() / 2), 0)
        tneigh_bottom = min(round(max_disp1[0].item() + target_neigh_sz_rgb[0].item() / 2 + 1), sz[0])
        tneigh_left = max(round(max_disp1[1].item() - target_neigh_sz_rgb[1].item() / 2), 0)
        tneigh_right = min(round(max_disp1[1].item() + target_neigh_sz_rgb[1].item() / 2 + 1), sz[1])
        ...
        translation_vec2_rgb = target_disp2 * (self.img_support_sz / self.output_sz) * self.target_scale_rgb

        # Mask out target ir neighborhood
        target_neigh_sz_ir = self.params.target_neighborhood_scale * self.target_sz_ir / self.target_scale_ir
        tneigh_top = max(round(max_disp1[0].item() - target_neigh_sz_ir[0].item() / 2), 0)
        tneigh_bottom = min(round(max_disp1[0].item() + target_neigh_sz_ir[0].item() / 2 + 1), sz[0])
        tneigh_left = max(round(max_disp1[1].item() - target_neigh_sz_ir[1].item() / 2), 0)
        tneigh_right = min(round(max_disp1[1].item() + target_neigh_sz_ir[1].item() / 2 + 1), sz[1])
        scores_masked = scores[scale_ind:scale_ind+1,...].clone()
        scores_masked[...,tneigh_top:tneigh_bottom,tneigh_left:tneigh_right] = 0
        ...
        max_disp2 = max_disp2.float().cpu().view(-1)
        target_disp2 = max_disp2 - self.output_sz // 2
        translation_vec2_ir = target_disp2 * (self.img_support_sz / self.output_sz) * self.target_scale_ir
        ...
        if max_score2 > self.params.distractor_threshold * max_score1:
            ...
            if disp_norm2 > disp_threshold and disp_norm1 < disp_threshold:
                return translation_vec1_ir, translation_vec1_rgb, scale_ind, scores, 'hard_negative'
            if disp_norm2 < disp_threshold and disp_norm1 > disp_threshold:
                return translation_vec2_ir, translation_vec2_rgb, scale_ind, scores, 'hard_negative'
            if disp_norm2 > disp_threshold and disp_norm1 > disp_threshold:
                return translation_vec1_ir, translation_vec1_rgb, scale_ind, scores, 'uncertain'
        ...
        return translation_vec1_ir, translation_vec1_rgb, scale_ind, scores, None

    def extract_sample(self, im_ir: torch.Tensor, im_rgb: torch.Tensor, pos_ir: torch.Tensor, pos_rgb: torch.Tensor, scales_ir, scales_rgb, sz: torch.Tensor):
        return self.params.features.extract(im_ir, im_rgb, pos_ir, pos_rgb, scales_ir, scales_rgb, sz)[0]
    ...
    def extract_processed_sample(self, im_ir: torch.Tensor, im_rgb: torch.Tensor, pos_ir: torch.Tensor, pos_rgb: torch.Tensor, scales_ir, scales_rgb, sz: torch.Tensor) -> (TensorList, TensorList):

        # x = self.extract_sample(im, pos, scales, sz)
        x = self.extract_sample(im_ir, im_rgb, pos_ir, pos_rgb, scales_ir, scales_rgb, sz)
    ...
    def generate_init_samples(self, im_ir: torch.Tensor, im_rgb: torch.Tensor) -> TensorList:
        """Generate augmented initial samples."""

        # Compute augmentation size
        aug_expansion_factor = getattr(self.params, 'augmentation_expansion_factor', None)
        aug_expansion_sz = self.img_sample_sz_ir.clone()
        aug_output_sz = None
        if aug_expansion_factor is not None and aug_expansion_factor != 1:
            aug_expansion_sz = (self.img_sample_sz_ir * aug_expansion_factor).long()
            aug_expansion_sz += (aug_expansion_sz - self.img_sample_sz_ir.long()) % 2
            aug_expansion_sz = aug_expansion_sz.float()
            aug_output_sz = self.img_sample_sz_ir.long().tolist()

        # Random shift operator
        get_rand_shift = lambda: None
        random_shift_factor = getattr(self.params, 'random_shift_factor', 0)
        if random_shift_factor > 0:
            get_rand_shift = lambda: ((torch.rand(2) - 0.5) * self.img_sample_sz_ir * random_shift_factor).long().tolist()
        ...
        if 'relativeshift' in self.params.augmentation:
            get_absolute = lambda shift: (torch.Tensor(shift) * self.img_sample_sz_ir/2).long().tolist()
        ...
        # im_ir im_rgb
        init_samples = self.params.features.extract_transformed(im_ir, im_rgb, self.pos_ir, self.pos_rgb, self.target_scale_ir, self.target_scale_rgb, aug_expansion_sz, self.transforms)
    ...
    def init_label_function(self, train_x):
        ...
        self.y_ir = TensorList([x.new_zeros(self.params.sample_memory_size, 1, x.shape[2], x.shape[3]) for x in train_x])

        # Output sigma factor
        output_sigma_factor = self.fparams.attribute('output_sigma_factor')
        self.sigma_ir = (self.feature_sz / self.img_support_sz * self.base_target_sz_ir).prod().sqrt() * output_sigma_factor * torch.ones(2)

        # Center pos in normalized coords
        target_center_norm_ir = (self.pos_ir - self.pos_ir.round()) / (self.target_scale_ir * self.img_support_sz)

        # Generate label functions
        for y_ir, sig, sz, ksz, x in zip(self.y_ir, self.sigma_ir, self.feature_sz, self.kernel_size, train_x):
            center_pos = sz * target_center_norm_ir + 0.5 * torch.Tensor([(ksz[0] + 1) % 2, (ksz[1] + 1) % 2])
            for i, T in enumerate(self.transforms[:x.shape[0]]):
                sample_center = center_pos + torch.Tensor(T.shift) / self.img_support_sz * sz
                y_ir[i, 0, ...] = dcf.label_function_spatial(sz, sig, sample_center)

        # Allocate label function
        self.y_rgb = TensorList([x.new_zeros(self.params.sample_memory_size, 1, x.shape[2], x.shape[3]) for x in train_x])

        # Output sigma factor
        self.sigma_rgb = (self.feature_sz / self.img_support_sz * self.base_target_sz_rgb).prod().sqrt() * output_sigma_factor * torch.ones(2)

        # Center pos in normalized coords
        target_center_norm_rgb = (self.pos_rgb - self.pos_rgb.round()) / (self.target_scale_rgb * self.img_support_sz)

        # Generate label functions
        for y_rgb, sig, sz, ksz, x in zip(self.y_rgb, self.sigma_rgb, self.feature_sz, self.kernel_size, train_x):
            center_pos = sz * target_center_norm_rgb + 0.5 * torch.Tensor([(ksz[0] + 1) % 2, (ksz[1] + 1) % 2])
            for i, T in enumerate(self.transforms[:x.shape[0]]):
                sample_center = center_pos + torch.Tensor(T.shift) / self.img_support_sz * sz
                y_rgb[i, 0, ...] = dcf.label_function_spatial(sz, sig, sample_center)

        # Return only the ones to use for initial training
        return TensorList([y_ir[:x.shape[0], ...] for y_ir, x in zip(self.y_ir, train_x)]), TensorList([y_rgb[:x.shape[0], ...] for y_rgb, x in zip(self.y_rgb, train_x)])
    ...
    def get_label_function(self, sample_pos, sample_scale):
        # Generate label function
        train_y = TensorList()
        target_center_norm = (self.pos_ir - sample_pos) / (sample_scale * self.img_support_sz)
    ...
    def update_state(self, new_pos_ir, new_pos_rgb, new_scale_ir = None, new_scale_rgb = None):
        # Update scale
        if new_scale_ir is not None:
            self.target_scale_ir = new_scale_ir.clamp(self.min_scale_factor_ir, self.max_scale_factor_ir)
            self.target_sz_ir = self.base_target_sz_ir * self.target_scale_ir

        if new_scale_rgb is not None:
            self.target_scale_rgb = new_scale_rgb.clamp(self.min_scale_factor_rgb, self.max_scale_factor_rgb)
            self.target_sz_rgb = self.base_target_sz_rgb * self.target_scale_rgb

        # Update pos
        inside_ratio = 0.2
        inside_offset = (inside_ratio - 0.5) * self.target_sz_ir
        self.pos_ir = torch.max(torch.min(new_pos_ir, self.image_sz_ir - inside_offset), inside_offset)
        inside_offset = (inside_ratio - 0.5) * self.target_sz_rgb
        self.pos_rgb = torch.max(torch.min(new_pos_rgb, self.image_sz_rgb - inside_offset), inside_offset)
