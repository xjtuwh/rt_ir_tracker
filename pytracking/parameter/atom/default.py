...
def parameters():
    ...
    params.init_CG_iter = 61            # The total number of Conjugate Gradient iterations used in the first frame
    params.init_GN_iter = 7             # The number of Gauss-Newton iterations used in the first frame (only if the projection matrix is updated)
    ...
    params.target_not_found_threshold = 0.25     # Absolute score threshold to detect target missing 0.25
    ...
    params.box_jitter_pos = 0.01              # How much to jitter the translation for random boxes
    params.box_jitter_sz = 0.02              # How much to jitter the scale for random boxes
    params.maximal_aspect_ratio = 3         # Limit on the aspect ratio
    params.box_refinement_iter = 5          # Number of iterations for refining the boxes
    params.box_refinement_step_length = 0.1   # Gradient step length in the bounding box refinement
    params.box_refinement_step_decay = 0.1    # Multiplicative step length decay (1 means no decay)

    # Setup the feature extractor (which includes the IoUNet)
    deep_fparams = FeatureParams(feature_params=[deep_params])

    # deep_feat = deep.ATOMResNet18(net_path='/media/hp/01a64147-0526-48e6-803a-383ca12a7cad/WH/wh2020/anti-UAV/Anti-UAV-master/Eval/ltr/save_models/checkpoints/ltr/bbreg/aog12/ATOMnet_ep0001.pth.tar', output_layers=['layer3'], fparams=deep_fparams,
    #                               normalize_power=2)
    deep_feat = deep.ATOMResNet18(net_path='/media/hp/01a64147-0526-48e6-803a-383ca12a7cad/WH/wh2019/pytracking/pytracking/networks/atom_default.pth', output_layers=['layer3'], fparams=deep_fparams,
                                  normalize_power=2)
    # deep_feat = deep.ATOMResNet18(net_path='/media/hp/01a64147-0526-48e6-803a-383ca12a7cad/WH/wh2020/anti-UAV/Anti-UAV-master/Eval/ltr/save_models/checkpoints/ltr/bbreg/resnet18_a/ATOMnet_ep0001.pth.tar', output_layers=['layer3'], fparams=deep_fparams,
    #                               normalize_power=2)
