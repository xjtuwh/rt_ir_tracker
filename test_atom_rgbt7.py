...
def main(mode='IR', visulization=False):
    ...
    output_dir = os.path.join('results', 'atom_expand2')
    output_video_path = '/media/hp/01a64147-0526-48e6-803a-383ca12a7cad/WH/wh2020/anti-UAV/visualize/' + 'atom_rgbt7/'
    ...
    for video_id, video_path in enumerate(video_paths, start=1):
        ...
        while True:
            ...
            if frame_id == 0:
                ...
                max_score_ir = 1
                max_score_rgb = 1
                sample_pos_rgb = tracker_rgb.pos
            else:
                tic = time.time()
                ...
                sample_pos_rgb = torch.Tensor([tracker.pos[0] / H1 * H2, tracker.pos[1] / W1 * W2]).round()
                ...
                # if max_score_ir < th_low * max_score_ref and max_score_rgb < th_low * max_score_ref:
                #     out = [0, 0, 0, 0]
            ...
            if visulization:
                ...
                if _exist:
                    cv2.rectangle(frame_RGB, (int(_gt_rgb[0]), int(_gt_rgb[1])), (int(_gt_rgb[0] + _gt_rgb[2]), int(_gt_rgb[1] + _gt_rgb[3])),
                                  (0, 255, 0))
                ...
                new_state_rgb = torch.cat(
                    (sample_pos_rgb[[1, 0]] - (tracker_rgb.target_sz[[1, 0]] - 1) / 2, tracker_rgb.target_sz[[1, 0]]))
                out_rgb = new_state_rgb.tolist()

                cv2.rectangle(frame_RGB, (int(out_rgb[0]), int(out_rgb[1])), (int(out_rgb[0] + out_rgb[2]), int(out_rgb[1] + out_rgb[3])),
                              (0, 255, 255))
